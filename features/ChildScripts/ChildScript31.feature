Feature: Application to validate Followup task with Manual Task

  # @ChildScript
  @ChildScript31
  #Createdby: Mary
  #CreatedDate:05/24/2023
  Scenario: Use case to validate Followup task with Manual Task
    Given user login to "Finlevit_admin" with "GroupAdmins" credentials
    And User get the Test data from "Usecase_TestData/ChildScript31" json file in "WorkFlow UI" screen
    ...
    And User will Edit the task "Review" and add Followup Task in "tasks" page
      ...
      | Validate Followup | Application Manager | 2        | 1         | 1           |
    Then User will Drag and Drop below task on "BackOfficeCanvas" in "tasks" screen
      | Widget | X   | Y   |
      | End    | 845 | 250 |
    ...
    # Validate Workflow UI - Follow up Yes Response with cancel (Follow-up task and original task both will be removed from my task/team task)
    ...
    And User select "span" tag drop down option "Yes" in "Followup GotResponse Dropdown" in "WorkFlow UI" screen
    And User select "span" tag drop down option "CANCEL" in "Followup Reason Dropdown" in "WorkFlow UI" screen
    And User Fill "Cancel the Follow up task" in "Followup Comment" in "WorkFlow UI" screen
    ...
    Then User validates Followup Tasks event for "Validate Followup" in "WorkFlow UI" screen in History page
      | Event                             |
      | Follow Up Task Creation Triggered |   
    Then User validates Followup Tasks event for "Validate Followup" in "WorkFlow UI" screen in History page
      | Event                             | 
      |Cancel the Follow up task|
    ...
    # And User wait until the spinner disappears
    And User get Transaction Id and save it in json file
    # Validate Workflow UI - Follow up Yes Response with Continue (Follow-up task and original task both will be displayed in my task/team task)
    Given user login to workflow UI "Finlevit_workflow" with "1" "AppManager" credentials
    And User wait until the "Filter Transaction id" appears in "WorkFlow UI" screen
    Then User click on "Button" "Home" in "HomePage" screen
    Then User will open "GroupName" group in "HomePage" screen
    ...
    And User select drop down option "Submit" in "Actions Dropdown" in "WorkFlow UI" screen
    And User select "span" tag drop down option "Yes" in "Followup GotResponse Dropdown" in "WorkFlow UI" screen
    And User select "span" tag drop down option "CONTINUE" in "Followup Reason Dropdown" in "WorkFlow UI" screen
    And User Fill "Continue the Follow up task" in "Followup Comment" in "WorkFlow UI" screen
    Then User click on "button" "OkButton" in "WorkFlow UI" screen
    And As "Application Manager" user verify Task "1" Followup task Details under "My Task" in "WorkFlow UI" screen
