Feature: Application to validate On change Populate action and configure data population for BO fields with reference of FO Dropdown

  @BODataPopulationFORefDropdown @ChildScript @ChildScript4
  #Createdby: Mary
  #CreatedDate:04/25/2023
  Scenario: Use case to demonstrate On change Populate action and configure data population for BO fields with reference of FO Dropdown
    Given user login to "Finlevit_admin" with "GroupAdmins" credentials
    And User get the Test data from "Usecase_TestData/ChildScript4" json file in "WorkFlow UI" screen
    ...
    And User Drag and Drop below widgets in "CanvasBody" in "UniqueWidgetFields" screen
      ...
      | Dropdown | 100 | 100 | Users     | Users      | ConfigureOptions |
    And User Drag and Drop below widgets in "CanvasBody" in "UniqueWidgetFields" screen
      | Widget | X   | Y   | FieldName     | FieldLabel      | Action | Status |
      | Button | 100 | 300 | GetEmpDetails | Get Emp Details | Submit | Save   |
      | Button | 300 | 300 | Submit        | Submit          | Submit | Save   |
    And User make "Users" field as require field in "UniqueWidgetFields" screen
    ...
    And User create Onload Triggers with "1" Rule Options Mapping "addOnloadTrigger" in "onLoadTrigger" screen
      ...
      | Get users | Get users   | EmployeeDetails | Get All EmployeeDetails | Rule1 | Pin  | EmployeeDetails | Users  | Pin    | Pin    |
    # Edit existing role
    ...
    # Drag Widgets onto the Canvas in Back Office
    Then User click on "span" "Save" in "Page" screen
    ...
    And User select drop down option "makula" in "Users" widget in "payload" screen
    And User wait for "1000"
