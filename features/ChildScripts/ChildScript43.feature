Feature: Validate Promote App from Intg to QA - App with API and Response task - validate Payload and WF
    # @ChildScript
    @ChildScript43
    ...
    Scenario: Use case to demonstrate API Task - Data for the API task is from FO, including the Unique field
        ...
        And User get the Test data from "Usecase_TestData/ChildScript43" json file in "WorkFlow UI" screen
        ...
        Then User will Drag and Drop below task on "BackOfficeCanvas" in "tasks" screen
            ...
            | Task   | 700 | 200 | Review   | Application Manager | 1           | Accept  | In Progress   |
        ...
        # # Promote App
        Given user login to "Finlevit_admin" with "GroupAdmins" credentials
        When User click on "tab" "Environment" in "HomePage" screen
        Then User verify connection to "HigherEnvironment" is available in "GroupSetting" screen
        When User click on "logo" "Home" in "HomePage" screen
        And User will open "GroupName" group in "HomePage" screen
        Then User click on "button" "group_settings" in "Groups" screen
        Then User click on "button" "DeploymentDashbaord" in "GroupSetting" screen
        And User selectes "Application_Name" and clicks on "Promote" button in "GroupSetting" screen
        And User should contain text "has been promoted" in "Pagelevel_alert_messgae" in "payload" screen
        Then User will "Logout" from "Page" the current session
        When User reloads the page

        Given user login to "hEnvURL_admin" with "Admin" credentials
        And User will open "GroupName" group in "HomePage" screen
        Then User will add "GroupAdmins" in "HomePage" screen
        When User click on "logo" "Home" in "HomePage" screen
        And User will open "GroupName" group in "HomePage" screen

        Then User will add "AppAdmins" in "Application" screen
        When User click on "logo" "Home" in "HomePage" screen
        And User will open "GroupName" group in "HomePage" screen
        Then User will search for "Application_Name" in "Application" screen
        # #add FO agent role
        ...
        Then User will "Logout" from "Page" the current session
        When User reloads the page
        Given user login to "hEnvURL_admin" with "GroupAdmins" credentials
        And User will open "GroupName" group in "HomePage" screen
        Then User will search for "Application_Name" in "Application" screen
        And User validate Payload app opening in a new tab
        Given user login to "Finlevit_admin" with "GroupAdmins" credentials
        ...
        And User get Transaction Id and save it in json file
        #WorkFlow UI
        Given user login to workflow UI "hEnvURL_workflow" with "1" "AppManager" credentials
        And User wait until the "Filter Transaction id" appears in "WorkFlow UI" screen
        Then User click on "Button" "Home" in "HomePage" screen
        Then User will open "GroupName" group in "HomePage" screen
        Then User will search for "Application_Name" in "Application" screen
