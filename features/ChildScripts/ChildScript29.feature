Feature: Validate Email failure task in WF
  ...
  Scenario: End to end till workflow for Email failure task in WF

    # #CreateGroup
    Given user login to "Finlevit_admin" with "GroupAdmins" credentials
    And User get the Test data from "Usecase_TestData/ChildScript29" json file in "WorkFlow UI" screen
    ...
    Then User will Drag and Drop below task on "BackOfficeCanvas" in "tasks" screen
      ...
      | Task   | 400 | 300 | Review1  | Application Manager | 1           | Accept  | In Progress   |

    Then User will Drag and Drop below task on "BackOfficeCanvas" in "tasks" screen
      | Widget | X   | Y   | TaskName | To     | Sub     | Template | FailureTaskRole     |
      | Email  | 600 | 300 | Email    | @Email | MailSub | MailBody | Application Manager |
 ...
 #     | SLA | Transaction Id | Task Name | Priority | Created At | Submitted By | Last Updated By | Last Updated At | Assigned To |
    ...
    Then User validate Task "2" Actions and select "Retry" action in "WorkFlow UI" screen
    # Task3
    And As "Application Manager" user verify Task "3" Details under "Team Task" in "WorkFlow UI" screen
    Then Based on the Task assigned to person logout or continue with the same user in "WorkFlow UI" screen for "Application_Name"
    ...
    Then User validates Tasks event and Action as "Retry" in "WorkFlow UI" screen
      | Task   |
      | Failed |
