Feature: Existing application for XML with API task and Response Task

    Background: Create File setup for Existing Application
        Given user login to "Finlevit_admin" with "Admin" credentials
        And User get the Test data from "Usecase_TestData/ChildScript76" json file in "WorkFlow UI" screen
        ...
        Then User will create app source setup with "File" in "appSourceSetup" screen
            ...
            | XML      | Primary    | 1QMF            | AutomationPrimary.xml |

    @ChildScript76 @ChildScript
    Scenario: Existing application for XML with API task and Response Task
        ...
        And User copy "ProcessType" save "ProcessType" in "ChildScript76_Payload" json file
        And User copy "FileName" save "SourceID" in "ChildScript76_Payload" json file
        ...
        And User will modify "DropDown" on "UniqueWidgetFields" screen
            | FieldName          | OptionType    | InputType | ValueCount | Value1 | Value2  | Value3  |
            | Countryof Residence | ManualOptions | Values    | 3          | India  | Albania | Algeria |
        ...
        # Given User get the Test data from "Usecase_TestData/ChildScript76_Payload" json file in "API" screen
        # And "Finlevit_admin" get the access token using "Access_Token" call
        # Then User submits transaction for "ChildScript76_Payload"
        # And User wait for "2000"
        # And User get the Test data from "Usecase_TestData/ChildScript76" json file in "WorkFlow UI" screen
